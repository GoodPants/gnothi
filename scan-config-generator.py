"""
This project is an attempt to create a tool which can generate configuration files for various network scanners and optionally allow a user to use the scans defined in the files to update a local inventory database. On first run it will identify what network interfaces on the host have IP addresses which are in private reserved IP ranges, then write the subnets for those IPs to a .json config file. Once this list of subnets is created, it uses entries in this file to build scanner configurations.

Ultimately, the objective is to be able to plop a Raspberry pi or something similar on a network and be able to do basic network asset discovery, inventory management, and vulnerability tracking from a single (simple) tool on that machine.
...
Feature wishlist:
* Use pycve to compare identified service versions with public CVEs in the NVD
* Build scanner configuration files based on the existing entries in the local database
"""
